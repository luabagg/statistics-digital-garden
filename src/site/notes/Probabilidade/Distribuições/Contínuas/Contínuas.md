---
{"dg-publish":true,"permalink":"/probabilidade/distribuicoes/continuas/continuas/","dg-note-properties":{}}
---



- **[[Probabilidade/Distribuições/Contínuas/Contínuas\|Contínuas]]**
	- [[Probabilidade/Distribuições/Contínuas/Densidade de Probabilidade\|Densidade de Probabilidade]]
	- [[Probabilidade/Distribuições/Contínuas/Distribuição Exponencial\|Distribuição Exponencial]]
	- [[Probabilidade/Distribuições/Contínuas/Distribuição F (Distribuição de Freira)\|Distribuição F (Distribuição de Freira)]]
	- [[Probabilidade/Distribuições/Contínuas/Distribuição Gama\|Distribuição Gama]]
	- [[Probabilidade/Distribuições/Contínuas/Distribuição Log-Normal\|Distribuição Log-Normal]]
	- [[Probabilidade/Distribuições/Contínuas/Distribuição Normal\|Distribuição Normal]]
	- [[Probabilidade/Distribuições/Contínuas/Distribuição Qui-Quadrado\|Distribuição Qui-Quadrado]]
	- [[Probabilidade/Distribuições/Contínuas/Distribuição T-Student\|Distribuição T-Student]]
	- [[Probabilidade/Distribuições/Contínuas/Distribuição Uniforme\|Distribuição Uniforme]]
	- [[Probabilidade/Distribuições/Contínuas/Distribuição Weibull\|Distribuição Weibull]]
	- [[Probabilidade/Distribuições/Contínuas/Função de Distribuição Acumulada\|Função de Distribuição Acumulada]]
	- [[Probabilidade/Distribuições/Contínuas/Função Gama\|Função Gama]]


